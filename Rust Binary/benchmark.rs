...
use crate::utils::{evaluate_benchmark_results, BasicPRNG, save_proof_to_file, save_proof_metadata, create_proof_summary};
use rayon::prelude::*;
...
const NUMBER_OF_LEAVES_TO_PROVE: usize = 10;  // Reduced from 5000 to 10
const NUM_RUNS: usize = 3;  // Reduced from 10 to 3
...
pub fn benchmark_verkle() {
    // Optimized for weaker machines - smaller tree sizes
    let num_keys = vec![1_024, 4_096, 16_384, 65_536];  // Reduced from larger sizes
    
    // Disable parallelization to reduce memory usage
    // num_keys.into_par_iter().for_each(|total_keys| {
    for total_keys in num_keys {
        ...
        let results: Vec<(Duration, Duration, usize)> = (0..NUM_RUNS).map(|i| {
            println!("Run {}", i);
            // Use the first leaves for proof generation
            ...
            // Save Verkle proof to file
            let proof_bytes = {
                // Create a simple serialization of proof components
                let mut fallback_data = Vec::new();
                // Save verification hint
                fallback_data.extend_from_slice(&proof.verification_hint.depths);
                // Save sorted commitments
                for comm in &proof.comms_sorted {
                    fallback_data.extend_from_slice(&comm.to_bytes());
                }
                fallback_data
            };
            
            save_proof_to_file(&proof_bytes, "verkle", total_keys, i, None);
            ...
            // Save proof metadata
            save_proof_metadata("verkle", total_keys, i, proof_time, verification_time, size, None);
    ...
    // Create Verkle proof summary
    let total_verkle_proofs = vec![1_024, 4_096, 16_384, 65_536].len() * NUM_RUNS;
    create_proof_summary("verkle", total_verkle_proofs, 0); // Size will be calculated from individual proofs
...
// Optimized for weaker machines
const NUM_KEYS: usize = 16;  // Reduced from 64 to 16
pub const KEY_LOG: usize = 4;  // log₂(16) = 4
const CAPACITY: usize = 12;  // Reduced from 14 to 12 (2^12 = 4096 gates)
...
pub fn benchmark_binary() {
    ...
    for i in 0..NUM_RUNS {
        ...
        for (index, &key) in proof_keys {
            ...
            // Save individual binary proof
            let proof_bytes = proof.to_bytes();
            save_proof_to_file(&proof_bytes, "binary", NUM_KEYS, i, Some(leaf_counter));
            
            // Save proof metadata for this individual proof
            save_proof_metadata("binary", NUM_KEYS, i, proving_time, verification_time, proof_bytes.len(), Some(leaf_counter));
    ...
    // Create Binary proof summary
    let total_binary_proofs = NUM_RUNS * NUMBER_OF_LEAVES_TO_PROVE;
    create_proof_summary("binary", total_binary_proofs, 0); // Size will be calculated from individual proofs
}
